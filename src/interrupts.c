...
struct IDT {
    struct interrupt_description_table entries[IDT_ENTRY_COUNT];
}
...
static void interrupt_print(void) {
    for(int i=0; i < IDT_ENTRY_COUNT; ++i) {
        struct interrupt_description_table *table = &idt->entires[i];
...
static uint64_t get_handler_address(struct interrupt_description_table* table) {
    return table->low_offset | (table->mid_offset << 16) | (table->high_offset << 32);
}

static void set_handler_address(struct interrupt_description_table* table, uint64_t addr) {
...
static void set_handler(struct IDT* table, intptr_t addr, enum gate_type type) {
